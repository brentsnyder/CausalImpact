...
CreateImpactPlot <- function(impact, metrics = c("original", "pointwise",
  ...
  q <- q + geom_rect(xmin = impact$model$post.period[1]-53, xmax = impact$model$post.period[2]-52,
  ...
  q <- q + geom_line(aes(y = mean), data,
                     size = 0.6, colour = "red", linetype = "solid")

  # Add observed data
  q <- q + geom_line(aes(y = response), size = 0.6, colour="blue")
  return(q)
}
